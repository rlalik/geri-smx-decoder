...
hidden if the `geri-smx-decoder_DEVELOPER_MODE` option is disabled. Enabling this
...
You have a few options to pass `geri-smx-decoder_DEVELOPER_MODE` to the configure
...
#### Targets `format-check` and `format-fix`
...
#### Targets `spell-check` and `spell-fix`
